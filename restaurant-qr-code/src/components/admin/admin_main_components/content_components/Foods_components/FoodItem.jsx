...
const FoodItem = ({
  ...
  const [editfileURL, setEditfileURL] = useState(food_image);
  const [foodImageFile, setFoodImageFile] = useState(food_image);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await axios.put(`https://webdev-backend-2e1ad2316dae.herokuapp.com/food/menu/${food_id}`, {
        food_name: editFoodName,
        food_url: editfileURL,
        food_description: editFoodDescription,
        category_id: editFoodCategory,
      });
      ...
      console.error("Error updating food item:"); // Custom message
      ...
      {editMode ? (
        <div className="mb-4">
          ...
          <input
            ...
            value={editfileURL}
            className="w-full px-2 py-1 border rounded-md focus:outline-none focus:border-blue-500 mt-2"
          />
        </div>
      ) : (
        // not editing
